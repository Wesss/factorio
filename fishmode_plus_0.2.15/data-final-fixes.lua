...
function denominate_fish(number)
  ...
  if number > 65535 then log(inspect(M.item_data.item_name)) end
  assert(number < 65536, "number of fish is too large")
  return {
    {name="raw-fish", amount=math.min(65535, number), type="item"}
  }
  -- local k = 64
...
local function get_ingredients_total_value(ingredients, recipe_name)
  ...
  total = item_ranking.item_data[ingredient_name].value * ingredient_amount
...
for item_name, item_data in pairs(item_ranking.item_data) do
  ...
  if item_recipe ~= "" then
    ...
    new_result.amount = new_result.amount * (#util.get_normalized_recipe_ingredients(item_recipe))
...
--[[
    if result_is_fluid then
      ...
      for i=#util.get_normalized_recipe_ingredients(item_recipe),2 , -1 do
