                ...
                if allowed == category then
                    if item_ranking.item_data[item.name] then
                        if item_ranking.item_data[item.name].value_mult then
                            ...
                            if (item.localised_description ~= "") and (item.localised_description ~= nil) then
                                item.localised_description = {
                                    "",
                                    item.localised_description, "\n",
                                    {"description.liquidate-multiplier", digit1, digit2, digit3}
                                }
                            else
                                item.localised_description = {"description.liquidate-multiplier", digit1, digit2, digit3}
                            end
                            --log(item_name)
                            --log("why")
                            --log(digit1)
